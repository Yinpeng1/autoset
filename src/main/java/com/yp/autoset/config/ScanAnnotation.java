...
//            Set<Class<?>> clsList = getClasses("com.yp.autoset");
//            if (clsList != null && clsList.size() > 0) {
//                for (Class<?> cls : clsList) {
//                    if (cls.getAnnotation(Yinpeng.class) != null) {
...
//    }
    ...
    public void setSingletonInMap() throws Exception{
        Set<Class<?>> clsList = getClasses("com.yp.autoset");
        if (clsList != null && clsList.size() > 0) {
            for (Class<?> cls : clsList) {
                if (cls.getAnnotation(Yinpeng.class) != null) {
//                    Map<Class<?>, Object> map = new HashMap<>();
                    //spring中根据配置的不同可以配置不同的代理
                    Enhancer enhancer = new Enhancer();
                    enhancer.setSuperclass(cls);
                    enhancer.setCallback(new CglibProxy());
                    map.put(cls, enhancer.create());
                }
            }
        }
    }
    ...
    public static void getMapping(String url, Map<String, String[]> paramMap) throws Exception{
        for (Class<?> cls : map.keySet()) {
            Method[] methods = cls.getMethods();
            for (Method method : methods) {
                method.setAccessible(true);
                RequestMapping annotation = method.getAnnotation(RequestMapping.class);
                Trainsaction trainsaction = method.getAnnotation(Trainsaction.class);
                if (annotation != null) {
                    ...
                    if (value.equals("/about")) {//判断是不是/about，是的话，就调用about(args)方法
                        ...
                        method.invoke(map.get(cls), "args");
                    }
                    if (value[0].equals(url) && !ObjectUtils.isEmpty(trainsaction)){
                        Parameter[] params = method.getParameters();
                        List<Object> args = new ArrayList(params.length);
                        for (Parameter parameter: params) {
                            System.out.println((parameter.getParameterizedType()).toString());
                            if ((parameter.getParameterizedType()).toString().contains("int")){
                                args.add(Integer.valueOf(paramMap.get(parameter.getName())[0]));
                            } else {
                                args.add((paramMap.get(parameter.getName())[0]));
                            }
                        }
                        method.invoke(map.get(cls), args.toArray());
                    }
                }
            }
        }
    ...
    public void autoSet() throws Exception{
        Set<Class<?>> clsList = getClasses("com.yp.autoset.controller");
        if (clsList != null && clsList.size() > 0) {
            for (Class<?> cls : clsList) {
...
//                        System.out.println(field.getGenericType());
                        field.set(map.get(cls), map.get(field.getType()));
                    }

                    if (field.getAnnotation(Autowired.class) != null){
                        field.setAccessible(true);
                        field.set(map.get(cls), SpringContextUtil.getBean(field.getName()));
                    }
                }
            }
        }
